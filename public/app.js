...
function addPerson(side = '찬성', name = '') {
  ...
  d.className = 'person';
  d.innerHTML = `<select><option>찬성</option><option>반대</option></select><input placeholder="학생 이름" value="${esc(name)}"><button title="삭제">×</button>`;
  d.querySelector('select').value = side;
  d.querySelector('button').onclick = () => {
    d.remove();
    renderSegments();
  };
  d.querySelector('input').oninput = () => renderSegments();
  d.querySelector('select').onchange = () => renderSegments();
  $('#people').append(d);
}
...
function speakerMap() {
  ...
  if (!speakers.length) {
    $('#speakerMap').innerHTML = '<span class="hint">무료 로컬 모드에서는 각 발언 구간의 학생을 아래에서 직접 지정합니다.</span>';
    return;
  }
...
function participantOptions(selected = '') {
  return `<option value="">학생 지정</option>${people().map(p =>
...
function renderSegments() {
  ...
  $('#segments').innerHTML = segments.map((s, i) => {
    ...
    return `<div class="segment">
      <div class="time">${time(s.start)}–${time(s.end)}</div>
      <div class="who">${manual ? '화자 미지정' : esc(s.speaker)}</div>
      <div class="segmentText">${esc(s.text)}</div>
  ...
  $$('.segmentPerson').forEach(sel => {
    sel.onchange = () => {
      ...
      segments[i].assignedName = sel.value;
    };
  });
  speakerMap();
}
...
function getSpeakerMap() {
  return Object.fromEntries(
    $$('#speakerMap select')
      .filter(x => x.value)
      .map(x => [x.dataset.speaker, x.value])
  );
}

function segmentsForEval() {
  return segments.map((s, i) => {
    const sel = document.querySelector(`.segmentPerson[data-index="${i}"]`);
    return { ...s, assignedName: sel?.value || s.assignedName || '' };
  });
}
...
async function transcribe() {
  ...
  const mode = provider();
  if (mode === 'local' && health && !health.providers?.local?.ready) {
    return toast('무료 로컬 엔진이 준비되지 않았습니다. Windows 로컬 설치를 먼저 완료해 주세요.');
  }
  if (mode === 'openai' && health && !health.providers?.openai?.ready) {
    return toast('OpenAI API 키가 설정되지 않았습니다.');
  }

  progress(true, mode === 'local'
    ? '무료 로컬 Whisper로 음성을 전사하고 있습니다. 긴 파일은 몇 분 이상 걸릴 수 있습니다.'
    : 'OpenAI로 음성을 화자별 전사하고 있습니다.');
  ...
  try {
    ...
    if (mode === 'local') {
      ...
      progress(true, '무료 로컬 Whisper로 음성을 전사하고 있습니다.');
    }
    ...
    renderSegments();
    toast(mode === 'local'
      ? '무료 로컬 전사가 완료되었습니다. 화자 미지정 구간은 학생 이름을 지정해 주세요.'
      : '화자 분리 전사가 완료되었습니다.');
...
function renderResult(d) {
  ...
  $('#resultBody').innerHTML = `
    ...
    ${d.clashes?.length ? `<div class="clashes"><h3>핵심 충돌 쟁점</h3>${d.clashes.map(x => `<div class="clash">
      <b>${esc(x.issue)}</b><p><b>찬성:</b> ${esc(x.pro)}</p><p><b>반대:</b> ${esc(x.con)}</p><p>${esc(x.assessment)}</p>
    </div>`).join('')}</div>` : ''}
    ${d.participants?.length ? `<div class="peopleResults"><h3>학생별 기여</h3>${d.participants.map(x => `<div class="personResult">
      <b>${esc(x.name)} · ${esc(x.side)}</b><p>${esc(x.contribution)}</p><p><b>강점:</b> ${esc(x.strength)}</p><p><b>개선:</b> ${esc(x.improvement)}</p>
    </div>`).join('')}</div>` : ''}
...
async function evaluate() {
  ...
  const ps = people();
  if (!ps.length) return toast('참가자 이름을 입력해 주세요.');
  ...
  if (auto.length && Object.keys(map).length === 0) {
    return toast('화자를 최소 한 명 이상 학생 이름과 연결해 주세요.');
  }

  if (!auto.length) {
    const unassigned = evalSegments.filter(s => !s.assignedName);
    if (unassigned.length) {
      return toast(`무료 로컬 전사에서 학생이 지정되지 않은 발언이 ${unassigned.length}개 있습니다.`);
    }
  }
...
function updateProviderInfo() {
  ...
  if (mode === 'local') {
    info.innerHTML = h?.ready
      ? `<b>무료 로컬 모드 준비됨</b><span>Whisper.cpp 전사 + Ollama ${esc(h.ollamaModel || '')} 평가 · API 비용 0원</span>`
      : `<b>무료 로컬 모드 설치 필요</b><span>Whisper ${h?.whisper ? '연결됨' : '미연결'} · Ollama ${h?.ollama ? '연결됨' : '미연결'}</span>`;
    $('#health').textContent = h?.ready ? '무료 로컬 준비됨' : '로컬 엔진 미설치';
  } else {
    info.innerHTML = h?.ready
      ? '<b>OpenAI 정밀 모드 준비됨</b><span>화자 분리 전사와 정밀 평가에 API 사용료가 발생합니다.</span>'
      : '<b>OpenAI API 키 미설정</b><span>Render 또는 .env에 OPENAI_API_KEY를 설정해야 합니다.</span>';
...
async function init() {
  ...
  $('#file').onchange = e => {
    ...
    $('#fileName').textContent = audioFile
  ...
  try {
    ...
    rubric = rr;
    $('#rubric').innerHTML = rr.sections.map(s => `<article>
      <b>${s.name} ${s.maxScore}점</b>
      ${s.criteria.map(c => `<p>• ${esc(c.name)} ${c.maxScore}점</p>`).join('')}
    </article>`).join('');
  } catch {
